...
class StockCrawlerGUI:
    ...
    def setup_ui(self):
        ...
        # 股票清單標籤
        stock_list_label = ttk.Label(
            left_frame, text="股票清單:", style="Header.TLabel"
        )
        ...
        scrollbar = ttk.Scrollbar(
            list_frame, orient="vertical", command=self.stock_tree.yview
        )
        ...
        deselect_all_btn = ttk.Button(
            button_frame, text="取消全選", command=self.deselect_all
        )
        ...
        self.result_text = scrolledtext.ScrolledText(
            result_frame, wrap=tk.WORD, height=25
        )
    ...
    def update_stock_tree(self, filtered_data=None):
        ...
        for stock in data_to_show:
            self.stock_tree.insert(
                "", "end", values=(stock["code"], stock["name"], stock["market"])
            )
    ...
    def filter_stocks(self, *args):
        ...
        for stock in self.stock_data:
            if (
                search_term in stock["code"].lower()
                or search_term in stock["name"].lower()
            ):
    ...
    def start_crawling(self):
        ...
        if len(selected_codes) > 10:
            if not messagebox.askyesno(
                "確認",
                f"您選擇了 {len(selected_codes)} 支股票，爬取可能需要較長時間，是否繼續？",
            ):
                return

        # 在新線程中執行爬蟲任務
        threading.Thread(
            target=self.crawl_stocks, args=(selected_codes,), daemon=True
        ).start()
    ...
    def crawl_stocks(self, stock_codes):
        ...
        try:
            ...
            # 建立 URL 清單
            urls = [
                f"https://www.wantgoo.com/stock/{code}/technical-chart"
            ...
            error_message = str(e)
            self.root.after(0, lambda: self.handle_crawling_error(error_message))
        finally:
