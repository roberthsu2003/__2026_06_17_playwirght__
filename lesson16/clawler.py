    ...
    try:
        ...
        if response.status == 200:
            ...
            if prods:
                ...
                result["price"] = int(digits) if digits else 0
...
async def fetch_momo(context: BrowserContext, keyword: str) -> Dict[str, Any]:
    ...
    💡 教學說明：
       ...
       3. 使用正則表達式 `re.sub(r"[^0-9]", "", price_text)` 清理價格字串（如 "NT$ 1,090" -> 1090）。
...
async def fetch_item_across_platforms(context: BrowserContext, brand: str, name: str, keyword: str) -> Dict[str, Any]:
    ...
    # 同時等待所有賣場回傳結果 (return_exceptions 確保單一賣場失敗不影響其他)
    store_results = await asyncio.gather(*stores_tasks, return_exceptions=True)
    store_results = [r for r in store_results if isinstance(r, dict)]
...
async def monitor_category_async(context: BrowserContext, category_item: Dict[str, Any]) -> Dict[str, Any]:
  ...
  # 同時執行毛寶與所有競品的抓取任務 (過濾掉失敗的 Exception)
...
async def main():
  ...
  with open(CONFIG_FILE, "r", encoding="utf-8") as f:
      ...
      async with async_playwright() as p:
        ...
        all_results = await asyncio.gather(*cat_tasks, return_exceptions=True)
